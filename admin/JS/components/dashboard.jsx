  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:12,animation:'bb-fadeUp .28s ease both'}}>
      ...
      <div style={card}>
        ...
        {[
          ['Phiên bản','Learnsy Admin v24.5'],
          ['Tác giả','EnglishFun · Việt Nam'],
          ['Stack','React · Supabase · Upstash · Cloudflare'],
...
function ResultsPanel({dark,C}){
  ...
  const toast=window._adminToast||alert;

  async function handleClearAll(){
    if(!results.length)return;
    const ok=window.confirm(`Xoá toàn bộ ${results.length} kết quả? Hành động này không thể hoàn tác.`);
    if(!ok)return;
    setClearing(true);
    try{
      const {error,count}=await window.supa.from('quiz_results').delete({count:'exact'}).not('id','is',null);
      if(error)throw error;
      // Verify: RLS có thể chặn DELETE âm thầm, trả về "thành công" nhưng count=0/rows vẫn còn
      const {data:remaining,error:checkErr}=await window.supa.from('quiz_results').select('id').limit(1);
      if(!checkErr&&remaining&&remaining.length>0){
        throw new Error('Dữ liệu vẫn còn sau khi xoá — có thể do quyền (RLS) chặn.');
      }
      setResults([]);
      toast(`Đã xoá toàn bộ kết quả!${count!=null?` (${count})`:''}`,'🗑️','#EF4444');
    }catch(e){
      console.warn('clear quiz_results:',e);
      toast('Xoá thất bại — kiểm tra quyền xoá (RLS) trên bảng quiz_results!','❌','#EF4444');
    }finally{
      setClearing(false);
    }
  }
      ...
      <div style={{display:'flex',justifyContent:'flex-end'}}>
        <button
          ...
          style={{
            ...
            color:'#EF4444',fontSize:11,fontWeight:800,fontFamily:'Nunito,sans-serif',
            opacity:clearing?.6:1,cursor:clearing?'not-allowed':'pointer',
          }}>
          {clearing?(
            <div style={{width:13,height:13,border:'2px solid rgba(239,68,68,.3)',borderTopColor:'#EF4444',borderRadius:'50%',animation:'bb-spin .6s linear infinite'}}/>
          ):(
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round">

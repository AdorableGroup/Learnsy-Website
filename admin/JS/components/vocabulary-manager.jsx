...
(function(){
  ...
  function CourseModal({dark, C, initial, onClose, onSaved, toast_}){
    ...
    return(
      <div onClick={e=>{if(e.target===e.currentTarget && !saving) onClose();}}
        ...
        <div style={{width:'100%', maxWidth:420, maxHeight:'85dvh', borderRadius:22, background:dark?'#1E0D15':'#fff', border:`1.5px solid ${C.border2}`, boxShadow:'0 24px 60px rgba(0,0,0,.3)', animation:'pop .2s ease both', display:'flex', flexDirection:'column', overflow:'hidden'}}>
  ...
  function WordModal({dark, C, unitId, initial, onClose, onSaved, toast_}){
    ...
    return(
      <div onClick={e=>{if(e.target===e.currentTarget && !saving) onClose();}}
        style={{position:'fixed', inset:0, zIndex:9200, background:'rgba(10,2,25,0.72)', backdropFilter:'blur(10px)', display:'flex', alignItems:'center', justifyContent:'center', padding:16}}>
        <div style={{width:'100%', maxWidth:440, maxHeight:'85dvh', borderRadius:22, background:dark?'#1E0D15':'#fff', border:`1.5px solid ${C.border2}`, boxShadow:'0 24px 60px rgba(0,0,0,.3)', animation:'pop .2s ease both', display:'flex', flexDirection:'column', overflow:'hidden'}}>
          <div style={{flex:1, overflowY:'auto', padding:'20px 18px 4px'}}>
            <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16}}>

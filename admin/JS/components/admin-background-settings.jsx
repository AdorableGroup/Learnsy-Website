...
(function () {
  ...
  async function upSave(s) {
    try {
      ...
      await window.upstashCmd('SET', UP_KEY, JSON.stringify(meta), 'EX', TTL);
      if (s.presetId === 'custom_image' && s.imageDataUrl) {
        await window.upstashCmd('SET', UP_IMG_KEY, s.imageDataUrl, 'EX', TTL);
      } else {
        await window.upstashCmd('DEL', UP_IMG_KEY);
      }
    } catch(e) { console.warn('[admin-bg-settings] upstash save failed', e); }
  }
  ...
  function adminApplyBackground(s, isDarkOverride) {
    ...
    const dimDark  = s.blurMode === 'blur85' ? 'rgba(10,0,20,0.72)'    : s.blurMode === 'blur50' ? 'rgba(10,0,20,0.42)'     : 'rgba(10,0,20,0.28)';
  ...
  function AdminBgSettingsCard({ dark }) {
    ...
    /* Lắng nghe dark mode auto-change */
    useEffect(() => {
      const handler = e => {

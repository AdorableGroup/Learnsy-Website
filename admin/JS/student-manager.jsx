...
function StudentManager({dark, C}){
  ...
  const doAdd = async () => {
    ...
    try {
      ...
      // Admin panel dùng cờ localStorage, không có session Supabase Auth thật,
      // nên phải tự gửi secret riêng để Edge Function xác thực (window.ADMIN_API_KEY
      // được set từ biến môi trường Cloudflare Pages lúc build/load).
  ...
  return (
    <div style={{padding:'14px 12px 100px'}}>
      ...
      {statusOpen && (
        <div style={{
          background:card, border:`1.5px solid ${bord}`, borderRadius:18, padding:14, marginBottom:16,
          ...
          {filtered.map(s => {
            ...
            <div key={s.id} style={{
              ...
              <div title="Click để đổi ảnh đại diện"
                ...
                {avatarUrls[s.id]
                  ? <img src={avatarUrls[s.id]} alt="" style={{width:'100%', height:'100%', objectFit:'cover', borderRadius:13, display:'block'}} onError={() => { localStorage.removeItem('ls_avatar_' + s.id); setAvatarUrls(p => { const n = {...p}; delete n[s.id]; return n; }); loadAvatar(s.id); }} />

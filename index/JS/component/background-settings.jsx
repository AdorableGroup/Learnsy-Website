...
(function () {
  ...
  function lsLoad(id) {
    try { const r = localStorage.getItem(lsKey(id)); return r ? JSON.parse(r) : defaultSettings(); }
  ...
  async function upSave(studentId, s) {
    try {
      ...
      // Lưu metadata (không kèm ảnh)
      const meta = { presetId: s.presetId, blurMode: s.blurMode };
      await window.upstashCmd('SET', upKey(studentId), JSON.stringify(meta), 'EX', TTL);
      // Lưu ảnh riêng nếu có
      if (s.presetId === 'custom_image' && s.imageDataUrl) {
        await window.upstashCmd('SET', upImgKey(studentId), s.imageDataUrl, 'EX', TTL);
      }
      // Không DEL upImgKey khi đổi sang preset khác — chỉ xoá khi user bấm "Xoá ảnh"
    } catch(e) { console.warn('[bg-settings] upstash save failed', e); }
  }
  ...
  function applyBackground(s, isDarkOverride) {
    ...
    const blurPx = s.blurMode === 'blur85' ? 22 : s.blurMode === 'blur50' ? 9 : 0;
    const dimLight = s.blurMode === 'blur85' ? 'rgba(255,255,255,0.45)' : s.blurMode === 'blur50' ? 'rgba(255,255,255,0.18)' : 'transparent';
    const dimDark  = s.blurMode === 'blur85' ? 'rgba(10,0,12,0.72)'    : s.blurMode === 'blur50' ? 'rgba(10,0,12,0.42)'     : 'rgba(10,0,12,0.32)';
  ...
  /* Lưu blurMode hiện tại trước khi dark mode ghi đè */
  function saveBlurBackup(mode) {
    try { localStorage.setItem(LS_KEY_BLUR_BACKUP, mode); } catch(e) {}
  }

  /* Lấy lại blurMode đã lưu; mặc định 'none' nếu chưa có */
  function loadBlurBackup() {
    try { return localStorage.getItem(LS_KEY_BLUR_BACKUP) || 'none'; } catch { return 'none'; }
  }
  ...
  new MutationObserver(() => {
    ...
    if (nowDark) {
      // Bật dark → lưu blurMode hiện tại rồi ép về 'off'
      if (s.blurMode !== 'off') saveBlurBackup(s.blurMode);
      ...
      // Tắt dark → khôi phục blurMode đã lưu
  ...
  function BgSettingsCard({ dark, studentId }) {
    ...
    /* Lắng nghe sự kiện dark mode tự động thay đổi blurMode */
    useEffect(() => {
      const handler = e => {
    ...
    return React.createElement('div', {
      ...
      React.createElement('div', { style:{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6, marginBottom:0 } },
        BLUR_MODES.map(m => {

  ...
  return (
    <div className="fade-up" style={{background: C.surface, ...blurStyle, border:`1.5px solid ${C.border}`, borderRadius: 18, overflow:'hidden', boxShadow:'0 3px 16px rgba(255,100,150,0.06)', borderTop:`3px solid ${accentColor}`}}>
      ...
      <div onClick={() => setOpen(p => !p)}
        ...
        <div style={{width:28, height:28, borderRadius:9, flexShrink:0, background:'linear-gradient(135deg,#F472B6,#A855F7)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:900, color:'#fff', boxShadow:'0 2px 8px rgba(168,85,247,0.25)'}}>
          {qi + 1}
        </div>
        <div style={{display:'flex', alignItems:'center', gap:5, flexShrink:0}}>
          <div style={{width:22, height:22, borderRadius:7, background: info.bg, border:`1.5px solid ${info.border}`, display:'flex', alignItems:'center', justifyContent:'center'}}>
            {info.icon}
          </div>
          <span style={{fontSize:10, fontWeight:900, padding:'2px 7px', borderRadius:999, background: info.bg, color: info.color, border:`1px solid ${info.border}`}}>{info.short}</span>
        </div>
        ...
        <div style={{display:'flex', gap:5, flexShrink:0}}>
          {autoAI && (
            <button onClick={e => { e.stopPropagation(); onAIAnswer?.(); }}
              ...
              style={{height:26, padding:'0 9px', borderRadius:8, border:'none',
          ...
          {canRemove && (
            <button onClick={e => { e.stopPropagation(); onRemove(); }}
              style={{width:26, height:26, borderRadius:8, border:`1.5px solid #FECDD3`, background: C.rosePale, color:'#EF4444', fontSize:14, fontWeight:900, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0}}>
              ×
            </button>
          )}
          <div style={{color: C.text4, fontSize:14, transition:'transform .2s', display:'inline-flex', alignItems:'center', transform: open ? 'rotate(180deg)' : 'rotate(0deg)'}}>
            <svg width="16" height="16" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 7l5 5 5-5"/></svg>
          </div>
        </div>
      </div>
      ...
      {open && (
        <div style={{padding:'11px 13px 14px', borderTop:`1px solid ${C.border}`}}>
                ...
                {q.items.map((it, ii) => (
                  <div key={ii} style={{display:'flex', gap:6, alignItems:'flex-start'}}>
                    ...
                    <div style={{display:'flex', gap:4, flexShrink:0, marginTop:4}}>
                      <button onClick={() => onUpItem(ii, 'answer', true)} style={{padding:'6px 10px', borderRadius:8, fontSize:12, fontWeight:800, cursor:'pointer', transition:'all .15s', background: it.answer ? '#10B981' : C.mintL, color: it.answer ? '#fff' : C.mint, border:`1.5px solid ${it.answer ? 'transparent' : '#BBF7D0'}`}}>✓</button>
                      <button onClick={() => onUpItem(ii, 'answer', false)} style={{padding:'6px 10px', borderRadius:8, fontSize:12, fontWeight:800, cursor:'pointer', transition:'all .15s', background: !it.answer ? '#EF4444' : C.rosePale, color: !it.answer ? '#fff' : '#EF4444', border:`1.5px solid ${!it.answer ? 'transparent' : '#FECDD3'}`}}>✗</button>
                      {q.items.length > 2 && <button onClick={() => onRemItem(ii)} style={{padding:'6px 8px', borderRadius:8, border:`1.5px solid ${C.border}`, background: C.bg, color: C.text4, fontSize:12, fontWeight:900, cursor:'pointer'}}>−</button>}
                    </div>
                  </div>
                ))}
              </div>
              <button onClick={onAddItem} style={{marginTop:8, padding:'5px 14px', borderRadius:999, border:`1.5px dashed ${C.lav2}`, background: C.lavL, color: C.lav, fontSize:12, fontWeight:800, cursor:'pointer'}}>＋ Thêm ý</button>
            </Fld>
          </>)}
          ...
          {(q.type === 'multiple' || q.type === 'multi_select') && (<>
            ...
            <Fld label={q.type === 'multiple' ? 'Lựa chọn — bấm chữ cái để chọn đáp án đúng' : 'Lựa chọn — bấm để chọn nhiều đáp án đúng'} icon={q.type === 'multiple' ? '◉' : '☑'}>
              <div style={{display:'flex', flexDirection:'column', gap:7}}>
                {q.options.map((opt, i) => {
                  ...
                  return (
                    <div key={i} style={{display:'flex', gap:7, alignItems:'center'}}>
                      <button onClick={togCor} style={{width:28, height:28, borderRadius: q.type === 'multiple' ? '50%' : 8, flexShrink:0, border:'none', cursor:'pointer', fontSize:12, fontWeight:900, transition:'all .15s', background: isCor ? '#10B981' : C.lavL, color: isCor ? '#fff' : C.lav, boxShadow: isCor ? '0 2px 8px rgba(16,185,129,0.3)' : undefined}}>
                        {LETTERS[i]}
                      </button>
                      <MiniRichInp value={opt} onChange={e => onUpOpt(i, e.target.value)} placeholder={`Lựa chọn ${LETTERS[i]}...`}/>
                      {q.options.length > 2 && <button onClick={() => onRemOpt(i)} style={{width:24, height:24, borderRadius:7, border:`1.5px solid ${C.border}`, background: C.bg, color: C.text4, fontSize:12, fontWeight:900, cursor:'pointer', flexShrink:0}}>×</button>}
                    </div>
                  );
                })}
              </div>
              {q.options.length < 6 && <button onClick={onAddOpt} style={{marginTop:8, padding:'5px 14px', borderRadius:999, border:`1.5px dashed ${C.lav2}`, background: C.lavL, color: C.lav, fontSize:12, fontWeight:800, cursor:'pointer'}}>＋ Thêm lựa chọn</button>}
            </Fld>
          </>)}

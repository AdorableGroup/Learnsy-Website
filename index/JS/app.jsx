...
const VocabPractice=window.VocabPractice;
...
function App(){
  ...
  const [student,setStudent]=useState(()=>{
    try{const s=localStorage.getItem('ls_student');return s?JSON.parse(s):null;}catch{return null;}
  });
  ...
  const doStudentLogin=useCallback(async(username,password)=>{
    try{
      ...
      try{localStorage.setItem('ls_student',JSON.stringify(info));}catch{}
      setStudent(info);
      return{ok:true};
  ...
  const doStudentLogout=useCallback(()=>{
    try{localStorage.removeItem('ls_student');}catch{}
    setStudent(null);
    setHistory([]);
        ...
        <>
          <div style={{display:homeTab==='lessons'?'block':'none'}}>
            <Dashboard
              ...
              onGoListening={()=>setHomeTab('listening')}
              onGoVocab={()=>setHomeTab('vocab')}
            />
          </div>
          {homeTab==='listening'&&ListeningPractice&&<ListeningPractice dark={dark} onBack={()=>setHomeTab('lessons')}/>}
          {homeTab==='vocab'&&VocabPractice&&<VocabPractice dark={dark} student={student} onBack={()=>setHomeTab('lessons')}/>}
        </>
      )}
